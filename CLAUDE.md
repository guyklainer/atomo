...
This file (`CLAUDE.md`) is the **sole authoritative source** for all protocol definitions, rules, criteria, and decision logic.
...
**TypeScript agent files must NEVER re-define or copy-paste content from this file inline.**
If you are modifying an agent and find yourself writing protocol logic (classification criteria, scoring weights, decision thresholds) inside a `.ts` file — stop. Add it here instead and reference it by name.
